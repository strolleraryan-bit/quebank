...
//
// Expects POST body: { action: 'objective' | 'subjective-questions' | 'subjective-answer', topic, question?, wordLimit?, lang? }
...
exports.handler = async (event) => {
  ...
  const { action, topic, question, wordLimit, lang, texts } = body;
  ...
  if (action === 'translate'){
    ...
    prompt = buildObjectivePrompt(topic, language);
    schema = objectiveSchema();
    maxOutputTokens = 8192;
    validate = (parsed) => Array.isArray(parsed?.questions) && parsed.questions.length >= 20 && parsed.questions.every(isValidObjectiveQuestion);
...
function buildObjectivePrompt(topic, language){
...
- Include at least 6 questions that are exact or closely adapted Previous Year Questions (PYQs) from UPSC CSE Prelims or UPPCS Prelims. Mark these with "pyq": true and name the exam and year in "pyqSource" (e.g. "UPSC CSE 2019", "UPPCS 2021"). For non-PYQ questions, omit "pyqSource" or leave it null.
...
- Do not repeat the same question twice.${languageInstruction(language)}
- The "questions" array must contain exactly 30 items.`;
}
