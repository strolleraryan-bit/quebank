...
async function generateWithRetry({ apiKey, prompt, schema, maxOutputTokens, validate }){
  ...
  // All attempts exhausted (or a non-retryable error occurred). Prefer a
  // quota-specific message for 429 over the raw per-attempt message — that
  // per-attempt message (e.g. "...Retrying…") describes an in-progress
  // attempt, not the terminal outcome, and would be misleading here since
  // by this point we've actually stopped retrying.
  const quotaExhausted = lastError && lastError.statusCode === 429;
  const finalErr = makeError(
    quotaExhausted
      ? 'Gemini API rate limit reached. Please wait a minute and try again, or check your API quota/billing in Google AI Studio.'
    ...
    const err = makeError(
      geminiRes.status === 429
        ...
        : nonRetryable
          ...
          : `Gemini API returned an error (${geminiRes.status}).`,
