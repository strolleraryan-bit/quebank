...
const I18N = {
  en: {
    ...
    'loading.buildingObjective': (topic) => `Building 30 MCQs on "${topic}"…`,
  ...
  hi: {
    ...
    'loading.buildingObjective': (topic) => `"${topic}" पर 30 MCQ तैयार किए जा रहे हैं…`,
...
async function callGenerate(payload){
  ...
  if (typeof window.QUEBANK_MOCK_GENERATE === 'function'){
    return window.QUEBANK_MOCK_GENERATE(body);
  }
...
async function generateObjectiveTopic(topic){
  showLoading(t('loading.buildingObjective', topic));
  try {
    const data = await callGenerate({ action: 'objective', topic });
    ...
    const entry = { id: uid(), topic, createdAt: Date.now(), questions, lastResult: null, history: [], flags: {}, tags: [], lang: currentLang, cache: {} };
    const list = loadObjTopics();
    list.unshift(entry);
    saveObjTopics(list);
    startTest(entry.questions, { title: topic, topicId: entry.id, isMix: false });
  } catch (err){
...
async function openObjectiveTopic(topicId){
  ...
  startTest(entry.questions, { title: entry.topic, topicId: entry.id, isMix: false });
}
...
function renderPalette(){
  ...
  testState.questions.forEach((q, i) => {
    ...
    dot.addEventListener('click', () => { testState.current = i; renderQuestion(); renderPalette(); });
    pal.appendChild(dot);
  });
}
...
function renderQuestion(){
  const q = testState.questions[testState.current];
  document.getElementById('qNumber').textContent = t('test.qNumber', testState.current + 1, testState.questions.length);
  ...
  document.getElementById('nextQBtn').disabled = testState.current === testState.questions.length - 1;
}

function moveQuestion(delta){
  const next = testState.current + delta;
  if (next < 0 || next >= testState.questions.length) return;
...
function showReport(result, title, questions, answers, meta){
  ...
  retakeBtn.addEventListener('click', () => {
    startTest(questions, { title, topicId: meta.topicId, isMix: meta.isMix });
  });
