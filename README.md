...
- **A 429's terminal error message wrongly said "Retrying…" even after retries had stopped.** `generateWithRetry`'s final-error construction preferred `lastError.userMessage` whenever it existed — but that message always exists (it's the per-attempt string, literally ending in "Retrying…"), so the friendlier quota-exhausted message added in the fix above was dead code; users saw "Retrying…" as the last thing shown even though the function had already given up. Fixed: the 429/quota-exhausted message now takes priority in the final error regardless of `lastError.userMessage`, and the per-attempt messages no longer say "Retrying…" at all, so no future intermediate message can be mistaken for a final one.
