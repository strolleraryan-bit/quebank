...
- **Client-side timeout + one silent retry** (`app.js`, `postWithRetry`) — every call to the Netlify function has its own 20s client-side timeout and retries once automatically on a transient failure before anything is shown to the user. The manual "Retry" button on the error screen remains as a fallback if both the server-side and client-side retries are exhausted.

If you need to tune this: `MAX_ATTEMPTS`, `ATTEMPT_TIMEOUT_MS`, and `BACKOFF_BASE_MS` are constants at the top of `netlify/functions/generate.js`; `REQUEST_TIMEOUT_MS` and `CLIENT_RETRY_DELAY_MS` are near the top of the API helper section in `app.js`.
